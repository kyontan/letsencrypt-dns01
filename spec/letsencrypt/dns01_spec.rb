...
describe Letsencrypt::Dns01 do
  ...
  describe Letsencrypt::Dns01::BIND9 do
    before do
      obj  = double('Challenges',        record_name: '_acme-challenge',
                                         record_type: 'TXT',
    ...
    let(:bind9) { Letsencrypt::Dns01::BIND9.new(cfg) }
    let(:bind_not_expire) { Letsencrypt::Dns01::BIND9.new(cfg_not_expire) }
    let(:fuga) { Letsencrypt::Dns01::Core.new(cfg) }

    it 'create new then return bind9' do
      expect(bind9).to be_a_kind_of(Letsencrypt::Dns01::BIND9)
    end

    it '#update' do
      fuga.update_cert(serial: '123')
      FileUtils.cp_r('spec/data/example.com/123/', 'spec/data/example.com/current')
      seri = bind9.instance_variable_get(:@serial)
      expect(bind9.update).to eq seri
    end
